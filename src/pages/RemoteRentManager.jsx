...
import { getRemoteProducts, createRemoteProduct, updateRemoteProduct, deleteRemoteProduct } from '../utils/api';

export default function RemoteRentManager() {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true);
      const data = await getRemoteProducts();
      setProducts(data);
    } catch (err) {
      console.error("Error loading Remote products", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await updateRemoteProduct(editId, payload);
        setProducts(products.map(p => p.id === editId ? { ...p, ...payload } : p));
      } else {
        const created = await createRemoteProduct(payload);
        setProducts([created, ...products]);
      }
      setShowForm(false);
      setEditId(null);
    } catch (err) {
      alert("Error saving Remote product: " + err.message);
    }
  };

  const handleDeleteClick = async (id) => {
    if (window.confirm("Are you sure you want to delete this Remote product?")) {
      try {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-900 pb-5">
        <div>
          <h1 className="text-xl md:text-2xl font-black uppercase tracking-tight text-gradient-gold">
            Remote &amp; Tool Rent Management
          </h1>
          <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mt-1.5">
            Add, edit, or remove remote access credentials, software tokens, and tool rents
          </p>
        </div>
        <button 
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass-card p-4 rounded-xl flex flex-col md:flex-row gap-4 items-center justify-between border border-slate-800 shadow-lg">
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Search Remote product name or details..."
                value={searchQuery}
            ...
            {loading ? (
              ...
              >
                {svc.image ? (
                  ...
                  <div className="w-12 h-12 bg-slate-805 rounded flex items-center justify-center text-[10px] text-slate-550 border border-slate-850">
                    RENT
                  </div>
                )}
                ...
                <div className="flex-1 flex flex-col justify-between min-w-0">
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                      <span className="bg-slate-850/80 border border-slate-700/60 text-slate-400 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded leading-none">
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-900 flex items-center justify-between">
                    <span className="text-xs font-black text-[#d4af37]">
                      ₹{svc.rentalPrice.toLocaleString('en-IN')}
                    </span>
        ...
        <div>
          <div className="glass-card p-6 rounded-2xl shadow-lg border border-slate-800 space-y-4">
            <h2 className="text-xs font-black text-slate-200 uppercase tracking-wider border-b border-slate-900 pb-3 flex items-center gap-2">
              ...
              {editId ? 'Modify Remote Product' : 'Create Remote Product'}
            </h2>
            ...
            {showForm ? (
              <form onSubmit={handleFormSubmit} className="space-y-4 text-xs">
                <div>
                  ...
                  <input
                    type="text" required
                    placeholder="e.g. UnlockTool Rent 24 Hours Key"
                    value={formName}
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="e.g. Rental, Credits"
                      value={formCategory}
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        type="text" readOnly
                        placeholder={formImage ? "Image Loaded" : "Upload..."}
                ...
                <div>
                  ...
                  <textarea
                    rows="3"
                    placeholder="Enter credentials delivery, tool download links..."
                    value={formDesc}
              ...
              <div className="py-12 text-center text-slate-555 flex flex-col items-center gap-2">
