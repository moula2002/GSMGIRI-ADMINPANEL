...
export default function RecentlyAddedManager({ services, setServices }) {
  const [searchQuery, setSearchQuery] = useState('');
  const [activeFilterTab, setActiveFilterTab] = useState('all');

  // Filter services into Recently Added vs others
  const recentServices = services.filter(svc => svc.isRecentlyAdded === true || svc.category === 'recent');
  const otherServices = services.filter(svc => !(svc.isRecentlyAdded === true || svc.category === 'recent'));

  // Search filter
  const filteredRecents = recentServices.filter(svc => {
  ...
  // Toggle handlers
  const handleToggleRecentlyAdded = async (svc, flag) => {
    const updatedSvc = { ...svc, isRecentlyAdded: flag };
    try {
      await updateService(svc.id, updatedSvc);
      setServices(services.map(s => s.id === svc.id ? updatedSvc : s));
    } catch (err) {
      alert(`Error updating recently added status: ${err.message}`);
    }
  };

  // Stats calculations
  const totalRecents = recentServices.length;
  const activeRecents = recentServices.filter(s => s.status === 'Active').length;
  const avgPrice = totalRecents > 0 
    ? Math.round(recentServices.reduce((sum, s) => sum + (s.priceINR || 0), 0) / totalRecents)
    : 0;
  ...
  const getServiceThumbnailPreview = (thumb, image) => {
    ...
    if (thumb === 'recent') {
      return (
        <div className={`${defaultClasses} bg-emerald-950/20 border-emerald-500/35 text-emerald-400 font-extrabold animate-pulse`}>
  ...
  return (
    <div className="space-y-8 text-[#111827] animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div className="glass-card rounded-xl p-5 border-l-4 border-l-emerald-500 shadow-md">
          ...
          <span className="text-[9px] text-emerald-500 font-semibold mt-1 block">
            Active products in customer lobby
          </span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Left/Middle Pane: Current New Arrivals (2 cols) */}
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredRecents.map((svc) => (
              <div 
                key={svc.id}
                className="glass-card rounded-2xl p-4 flex gap-4 border border-emerald-500/25 hover:border-emerald-500/60 transition-all group shadow-md bg-gradient-to-br from-emerald-500/2 to-transparent"
              >
